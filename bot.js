...
client.on('message', msg => {
  if (msg.content.toLowerCase() === 'çömez') {  // İstediğiniz Komut
       message.member.addRole("535206204285124618") //Rolü bir yerde bahsedin sonra sağ tıklayıp İD'sini alın
    msg.reply('Çömez Rolünü Başarıyla Aldın.'); //Komutu Yazınca cevap ne yazsın?
  }
});
...
client.on("guildMemberAdd", async member => {
  const channel = member.guild.channels.find('name', 'giriş-çıkış'); // buraya sunucunuzda gelen gideni yazmasını istediğiniz kanalın ismini yazın mesajı oraya atar
  if (!channel) return;
